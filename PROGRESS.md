...
| 1 | DB 계층(tauri-plugin-sql + 마이그레이션) + 과목 CRUD/관리 화면 | ⬜ 예정 |
...
src/
  ...
  windows/                 # MainApp / TimerOverlay / QuickStart (플레이스홀더)
  lib/utils.ts             # cn()
src-tauri/
  src/{lib.rs, main.rs}    # 플러그인 초기화 (commands.rs/state.rs/tray.rs는 단계 2·7에서 추가)
  capabilities/{main,timer,quickstart}.json
...
CLAUDE.md                  # 개발 가이드·규약
```
※ `migrations/`(단계 1), `src/hooks/`·`src/lib/{db,ipc,types}.ts`(단계 1), `src/components/ui/`(shadcn 컴포넌트, 필요 시)는 이후 단계에서 생성.
