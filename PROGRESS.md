...
- 기획 로드맵 원본: [`docs/02-기획-v1-핵심기능.md`](./docs/02-기획-v1-핵심기능.md) §11
...
| 15 | **버그 수정**: 첫 실행 시 통계 로딩 실패(DB 연결 경합) — `getDb` 재시도·실패 캐시 방지 · v4.0.1 | ✅ 완료 |
| 16 | **문의 탭**: 로그인 없이 앱 내 폼으로 버그·개선 문의 전송(Web3Forms → 개발자 이메일, 앱버전·환경 자동 첨부) · v4.1.0 | ✅ 완료 |

- v2 기획: [`docs/03-기획-v2-자동복구-백업-릴리스.md`](./docs/03-기획-v2-자동복구-백업-릴리스.md)
...
### ✅ 단계 16 — 문의 탭 (개발자 문의) (2026-07-18)
버전 **4.0.2 → 4.1.0**(신규 기능). 사이드바에 **"문의"** 탭을 추가해, 사용자가 **로그인 없이** 앱 안에서 버그·개선 제안을 보낼 수 있게 함.

**배경**: 서버 없는 로컬 앱이라 "누구나 로그인 없이 문의"를 만들려면 외부 채널이 필요. 앱을 떠나지 않고(메일앱 미설정 사용자도 가능) 개발자 이메일로 바로 배달되도록 **폼백엔드 Web3Forms**를 택함(계정/대시보드 없이 이메일만 등록 → 공개용 access key, 무료 250건/월). 이미 깔린 `tauri-plugin-http`로 그대로 POST.

**한 일**
- **전송 로직**(`lib/support.ts` 신설): `submitInquiry()`가 유형·제목·내용·(선택)이메일을 Web3Forms(`api.web3forms.com/submit`)로 POST. **앱 버전(`getVersion`)·환경(`navigator.userAgent`)을 자동 첨부**해 버그 리포트 품질↑. 이메일 입력 시 Web3Forms의 `email`/`replyto`로 답장 주소 지정, 비우면 익명. `access_key`는 **공개용**(클라이언트 임베드 설계)이라 상수로 둠 — 단, **배포 전 실제 키로 교체 필요**(플레이스홀더면 `isSupportConfigured()`가 false → 화면이 "준비 중" 안내 + 전송 차단). 네트워크/서비스 오류는 예외로 던져 toast.
- **문의 화면**(`components/support/SupportScreen.tsx` 신설): 유형 선택(버그/기능제안/기타) + 제목 + 내용(textarea, 글자수) + 답장 이메일(선택, 형식 검증) + 자동 첨부 안내. 제목·내용(≥5자)·이메일 형식 충족 시에만 "보내기" 활성. 성공 시 폼 초기화 + toast. 설정 화면의 `Section`/스타일 재사용.
- **탭 배선**(`windows/MainApp.tsx`): `Screen`에 `"support"` 추가, 사이드바 NAV에 `LifeBuoy` 아이콘 "문의" 항목, 라우팅 분기 + 사이드바 버전 v4.0.1→**v4.1.0**.
...
**증상**: 앱을 종료 후 첫 실행하면 대시보드가 "통계를 불러오지 못했습니다"를 띄우고, **새로고침해야** 정상 표시.

**원인**(경합 + 실패 캐싱, 둘 다 `lib/db.ts` `getDb()`):
- 웹뷰가 Tauri SQL 플러그인(+마이그레이션) 준비보다 먼저 떠서, 첫 순간의 `Database.load()`가 실패 → `useStats`가 잡아 에러 카드 표시. 새로고침 시 웹뷰 리로드로 재연결돼 정상.
- 싱글턴이 **실패한 promise까지 캐싱**해, 첫 로드 실패 시 이후 모든 DB 접근(`session-saved` 재조회 등)이 새로고침 전까지 같은 실패를 영구 재사용.

**한 일**(`src/lib/db.ts` 한 곳):
- `Database.load()`를 짧은 백오프(100→200→400→800→1200ms) 최대 6회 재시도 → 플러그인 준비까지 대기. 정상 실행은 대부분 즉시 성공이라 영향 없음.
- 연결 실패 시 `dbPromise = null`로 캐시를 비워 다음 호출이 새로 시도하게 함.
- `getDb()`는 모든 DB 소비자(통계·과목·세션·설정·백업)의 공통 진입점이라 앱 전체 첫 실행 견고성이 함께 개선.

**검증 결과**
- `npm run build` ✅ (tsc + vite, 2276 모듈) · `cargo check` ✅ (`study-log v4.0.1`).
- ⏳ 런타임 E2E는 **사용자 테스트 대기** — 앱 재실행 후 **새로고침 없이** 대시보드 통계가 바로 뜨는지 확인.
...
버전 **3.1.1 → 3.2.0**(신규 기능). 상세: [`docs/06-자동-업데이트-v3.2.0.md`](./docs/06-자동-업데이트-v3.2.0.md).
...
- ⏳ 런타임 E2E는 **사용자 테스트 대기** — 절차는 [`docs/06-자동-업데이트-v3.2.0.md`](./docs/06-자동-업데이트-v3.2.0.md) "검증 안내"(v3.2.0 설치 → 다음 태그 릴리스 → 재시작 시 팝업·업데이트·재시작).

### ✅ 단계 12 — 안정성 보강 (코드리뷰 후속) (2026-07-18)
버전 **3.1.0 → 3.1.1**(버그·안정성 패치, 기능 추가 없음). 상세: [`docs/05-안정성-보강-v3.1.1.md`](./docs/05-안정성-보강-v3.1.1.md).
...
- ⏳ 런타임 E2E는 **사용자 테스트 대기** — 체크리스트는 [`docs/05-안정성-보강-v3.1.1.md`](./docs/05-안정성-보강-v3.1.1.md) "검증 안내" 참고(복원 손상본 방어·리로드 강제휴식·핫키 연속저장·멀티모니터 복귀·Alt+F4·히트맵/표 표시).
...
**기획**: [`docs/03-기획-v2-자동복구-백업-릴리스.md`](./docs/03-기획-v2-자동복구-백업-릴리스.md) (F1~F4). 버전 **1.0.0 → 2.0.0** 승격(`tauri.conf.json`/`Cargo.toml`/`package.json`/사이드바).
...
## 다음 세션 시작 가이드
1. 이 문서 + `CLAUDE.md` + `docs/02-기획-v1-핵심기능.md` 확인.
